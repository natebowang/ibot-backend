...
app.use(session({
    ...
    saveUninitialized: false, // 如果为true，空session {}也会被存储到redis，然后给Client一个sessionID。如果为false，session如果为空，request结束session就删除了，也不会给客户发sessionID。false可以避免存储很多空object。true的话便于跟踪用户。
